...
function parseDateKey(dateKey) {
    // Keep date-key parsing anchored inside the Eastern calendar day so
    // formatting in America/New_York never falls back to the prior date.
...
function defaultUserState(userId) {
    return {
        ...
        lastActivityAt: null,
        platform: null,
        mobilePresenceEnabled: false,
        mobilePresenceUpdatedAt: null,
        mobileOnCall: false,
        mobileOnCallUpdatedAt: null,
        lastUpdated: null,
    };
}
...
function normalizeManualDailySeconds(value) {
    ...
    return Object.fromEntries(
        Object.entries(value)
            .filter(([dateKey, seconds]) => /^\d{4}-\d{2}-\d{2}$/.test(dateKey) && Number.isFinite(Number(seconds)) && Number(seconds) >= 0)
            .map(([dateKey, seconds]) => [dateKey, Math.floor(Number(seconds))]),
    );
}

function getManualDailySecondsForDateKey(state, dateKey) {
    const manualDailySeconds = normalizeManualDailySeconds(state?.manual_daily_seconds);
    return Object.hasOwn(manualDailySeconds, dateKey) ? manualDailySeconds[dateKey] : null;
}

function buildTrackedSecondsByDateKey(records) {
    return records.reduce((acc, record) => {
        const normalizedRecord = normalizeLogRecord(record);
        if (normalizedRecord.countsTowardTime) {
            acc[normalizedRecord.dateKey] = (acc[normalizedRecord.dateKey] || 0) + TRACKING_INTERVAL_SECONDS;
        }
        return acc;
    }, {});
}

function getAdjustedTrackedSecondsForDateKey(dateKey, trackedSecondsByDateKey, state) {
    const manualSeconds = getManualDailySecondsForDateKey(state, dateKey);
    return manualSeconds ?? Number(trackedSecondsByDateKey[dateKey] || 0);
}

function sumAdjustedTrackedSeconds(dateKeys, trackedSecondsByDateKey, state) {
    return dateKeys.reduce(
        (total, dateKey) => total + getAdjustedTrackedSecondsForDateKey(dateKey, trackedSecondsByDateKey, state),
        0,
    );
}
...
async function deleteKeys(keys) {
    ...
    for (let i = 0; i < filteredKeys.length; i += 1000) {
        const batch = filteredKeys.slice(i, i + 1000);
...
async function saveUserState(userId, updates) {
    ...
    const nextState = {
        ...
        manual_daily_seconds: normalizeManualDailySeconds(updates?.manual_daily_seconds ?? currentState.manual_daily_seconds),
        ...
        onCall: Boolean(updates?.onCall ?? currentState.onCall ?? false),
        onCallCheckinDueAt: normalizeIsoTimestamp(Object.hasOwn(updates || {}, 'onCallCheckinDueAt') ? updates.onCallCheckinDueAt : currentState.onCallCheckinDueAt),
        onCallCheckinActive: Boolean(updates?.onCallCheckinActive ?? currentState.onCallCheckinActive ?? false),
        onCallCheckinDeadline: normalizeIsoTimestamp(Object.hasOwn(updates || {}, 'onCallCheckinDeadline') ? updates.onCallCheckinDeadline : currentState.onCallCheckinDeadline),
        onCallCheckinConfirmedAt: normalizeIsoTimestamp(Object.hasOwn(updates || {}, 'onCallCheckinConfirmedAt') ? updates.onCallCheckinConfirmedAt : currentState.onCallCheckinConfirmedAt),
        ...
        mobilePresenceEnabled: Boolean(updates?.mobilePresenceEnabled ?? currentState.mobilePresenceEnabled ?? false),
        mobilePresenceUpdatedAt: normalizeIsoTimestamp(Object.hasOwn(updates || {}, 'mobilePresenceUpdatedAt') ? updates.mobilePresenceUpdatedAt : currentState.mobilePresenceUpdatedAt),
        mobileOnCall: Boolean(updates?.mobileOnCall ?? currentState.mobileOnCall ?? false),
        mobileOnCallUpdatedAt: normalizeIsoTimestamp(Object.hasOwn(updates || {}, 'mobileOnCallUpdatedAt') ? updates.mobileOnCallUpdatedAt : currentState.mobileOnCallUpdatedAt),
...
async function deleteLogsByIds(ids) {
    ...
    const records = (await Promise.all(uniqueIds.map((id) => getLogById(id)))).filter(Boolean);
    ...
    for (const record of records) {
        if (record.imageKey) keysToDelete.push(record.imageKey);
        if (record.logKey) keysToDelete.push(record.logKey);
        keysToDelete.push(logIndexKey(record._id));

        if (record.countsTowardTime) {
            countsByUser[record.userId] = (countsByUser[record.userId] || 0) + 1;
        }
    }
    ...
    for (const [userId, count] of Object.entries(countsByUser)) {
        await adjustAllTimeAutoCount(userId, -count);
    }
...
async function getTrackingStats(userId, currentDate = new Date()) {
    ...
    const weekDateKeys = getDateKeysInRange(weekStartKey, currentDateKey);
    const weekTrackedSecondsByDateKey = buildTrackedSecondsByDateKey(weekLogs);
    const todayTrackedSecondsByDateKey = buildTrackedSecondsByDateKey(todayLogs);
    const manualOverrideKeys = Object.keys(normalizeManualDailySeconds(state.manual_daily_seconds));
    const manualOverrideLogs = manualOverrideKeys.length > 0
        ? await Promise.all(manualOverrideKeys.map((dateKey) => listLogsForDate(normalizedUserId, dateKey)))
        : [];
    const manualOverrideTrackedSecondsByDateKey = buildTrackedSecondsByDateKey(manualOverrideLogs.flat());
    const manualOverrideDeltaSeconds = manualOverrideKeys.reduce((total, dateKey) => {
        const rawSeconds = Number(manualOverrideTrackedSecondsByDateKey[dateKey] || 0);
        const adjustedSeconds = getAdjustedTrackedSecondsForDateKey(dateKey, manualOverrideTrackedSecondsByDateKey, state);
        return total + (adjustedSeconds - rawSeconds);
    }, 0);
    ...
    return {
        ...
        weekAutoCount,
        allTimeAutoCount,
        todaySeconds: getAdjustedTrackedSecondsForDateKey(currentDateKey, todayTrackedSecondsByDateKey, state),
        weekSeconds: sumAdjustedTrackedSeconds(weekDateKeys, weekTrackedSecondsByDateKey, state),
        allTimeSeconds: (allTimeAutoCount * TRACKING_INTERVAL_SECONDS) + manualOverrideDeltaSeconds,
        manual: {
...
function buildPresenceSummaryFromState(state, now = new Date()) {
    ...
    const desktopHeartbeatMs = state.lastHeartbeatAt ? new Date(state.lastHeartbeatAt).getTime() : 0;
    const mobilePresenceMs = state.mobilePresenceUpdatedAt ? new Date(state.mobilePresenceUpdatedAt).getTime() : 0;
    const effectiveHeartbeatAt = state.mobilePresenceEnabled && mobilePresenceMs > desktopHeartbeatMs
        ? state.mobilePresenceUpdatedAt
        : state.lastHeartbeatAt;
    const heartbeatAgeSeconds = getSecondsSince(effectiveHeartbeatAt, nowMs);
    const desktopHeartbeatAgeSeconds = getSecondsSince(state.lastHeartbeatAt, nowMs);
    ...
    const desktopRecentlySeen = desktopHeartbeatAgeSeconds !== null && desktopHeartbeatAgeSeconds <= PRESENCE_STALE_SECONDS;
    const desktopOnline = Boolean(state.isOnline && desktopRecentlySeen);
    const isOnline = Boolean(desktopOnline || state.mobilePresenceEnabled);
    const isTracking = Boolean(desktopOnline && state.isTracking);
    const isManualPresence = Boolean(state.mobilePresenceEnabled && !isTracking);
    const isManualOnCall = Boolean(isManualPresence && state.mobileOnCall);
    const manualPresenceSince = isManualPresence ? state.mobilePresenceUpdatedAt : null;
    const manualPresenceDurationSeconds = manualPresenceSince ? getSecondsSince(manualPresenceSince, nowMs) : null;
    const manualOnCallSince = isManualOnCall ? state.mobileOnCallUpdatedAt : null;
    const onCall = Boolean(isTracking && state.onCall);
    const inferredIdle = Boolean(!onCall && isTracking && activityAgeSeconds !== null && activityAgeSeconds >= IDLE_THRESHOLD_SECONDS);
    ...
    if (isManualOnCall) {
        status = 'manual-on-call';
        statusLabel = 'On Call';
    } else if (isManualPresence) {
    ...
    let onCallCheckinSecondsRemaining = null;
    if (onCall || isManualOnCall) {
        if (state.onCallCheckinActive && state.onCallCheckinDeadline) {
            onCallCheckinSecondsRemaining = Math.max(0, Math.ceil((new Date(state.onCallCheckinDeadline).getTime() - nowMs) / 1000));
        } else if (state.onCallCheckinDueAt) {
            onCallCheckinSecondsRemaining = Math.max(0, Math.ceil((new Date(state.onCallCheckinDueAt).getTime() - nowMs) / 1000));
        }
    }
    ...
    return {
        ...
        isManualOnCall,
        isIdle,
        onCall: Boolean(onCall || isManualOnCall),
        onCallCheckinDueAt: state.onCallCheckinDueAt || null,
        onCallCheckinActive: Boolean((onCall || isManualOnCall) && state.onCallCheckinActive),
        onCallCheckinDeadline: state.onCallCheckinDeadline || null,
        onCallCheckinSecondsRemaining,
        onCallCheckinConfirmedAt: state.onCallCheckinConfirmedAt || null,
        platform: state.platform || null,
        trackingStartedAt: state.trackingStartedAt || null,
        manualPresenceSince,
        manualPresenceDurationSeconds,
        manualOnCallSince,
        activeSince: state.activeSince || null,
        idleSince: state.idleSince || null,
        lastHeartbeatAt: effectiveHeartbeatAt || null,
...
async function resetOnCallCheckin(userId) {
    const normalizedUserId = normalizeUserId(userId);
    const now = new Date();
    const dueAt = new Date(now.getTime() + 30 * 60 * 1000).toISOString();
    return saveUserState(normalizedUserId, {
        onCallCheckinActive: false,
        onCallCheckinDeadline: null,
        onCallCheckinDueAt: dueAt,
        onCallCheckinConfirmedAt: now.toISOString(),
